 ...
 * Supports:
 * - Data-attributes from DOM element (Astro SSR) - highest priority
 * - Query params: ?user=handle&post=postId
 * - Path-based: /@handle/postId
 * - Direct path: /handle/postId
 */
function parsePostURL() {
  // First check for data-attributes from Astro SSR (highest priority)
  const postEl = document.getElementById('post');
  if (postEl && postEl.dataset) {
    const handle = postEl.dataset.handle;
